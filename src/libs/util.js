...
export const getMenuByRouter = (list, access) => {
   //console.log(JSON.stringify(list)+"===list=access==>>>>")
  let res = []
  forEach(list, item => {
...
export const canTurnTo = (name, access, routers) => {
  ...
  return routePermissionJudge(routers)
}
...
export const formatMenu = (list) => {
  ...
  forEach(list, item => {
    ...
    if(item.hideInMenu=== 'true'){
      obj.meta={hideInMenu: true ,title: item.title}
    }else if(item.showAlways ==='true'){
      obj.meta={icon: item.icon , showAlways: true ,title: item.title}
    }else{
      obj.meta={icon: item.icon,title: item.title}
    }
    if(item.component === 'Main'){
      ...
      obj.component = () => import('@/view' + data+'.vue')
    }
    if(hasChild(item)){
